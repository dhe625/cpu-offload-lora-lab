...
class LoRAModelManager:
    ...
    def _load_weights(self, lora_dir: str):
        ...
        with safe_open(path, framework='pt') as f:
            """
            TODO: implement loading of LoRA weights in three steps:
            1) Load transformer layer weights (parts[3] == 'layers').
            2) Load embedding layer weights (parts[3] == 'embed_tokens').
            3) Load language model head weights (parts[2] == 'lm_head').
            """
            for key in f.keys():
                parts = key.split('.')
