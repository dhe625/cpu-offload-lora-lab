...
def main():
    ...
    parser.add_argument("--lora-mode", required=True,
                        help="0: LoRA using single stream, 1: LoRA using multi stream, 2: LoRA using CPU offload")
    ...
    lora_mode = int(args.lora_mode)
    if lora_mode == 2:
        lora_device = "cpu"
    else:
        lora_device = device

    wrap_model(model, lora_dir=lora_dir, lora_mode=lora_mode, lora_device=lora_device, skip_embed=False)
